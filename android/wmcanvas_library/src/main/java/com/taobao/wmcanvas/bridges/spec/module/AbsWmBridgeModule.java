package com.taobao.wmcanvas.bridges.spec.module;
...
import com.taobao.wmcanvas.WmCanvasJNI;
import com.taobao.wmcanvas.adapters.img.IWmImageLoader;
import com.taobao.wmcanvas.bridges.spec.bridge.IJSCallbackDataFactory;
import com.taobao.wmcanvas.bridges.spec.bridge.IJSCallbackMap;
import com.taobao.wmcanvas.util.WmLog;
...
public abstract class AbsWmBridgeModule<JSCallback> implements IWmBridgeModule<JSCallback> {
    ...
    public static final String TAG = AbsWmBridgeModule.class.getSimpleName();

    protected HashMap<String, WmImageLoadInfo> mImageIdCache = new HashMap<>();

    protected HashMap<String, ArrayList<JSCallback>> mCallbacks = new HashMap<>();

    protected IWmImageLoader mImageLoader;

    public void setImageLoader(IWmImageLoader mImageLoader) {
        this.mImageLoader = mImageLoader;
    }

    private abstract static class AbsImageCallback implements IWmImageLoader.ImageCallback {
        ...
        if (!TextUtils.isEmpty(src)) {
            try {
                if (src.startsWith("data:image")) {
                    ...
                    if (bmp != null) {
                        WmCanvasJNI.bindTexture(canvasId, bmp, 0, GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE);
                    } else {
                        WmLog.d("decode base64 texture failed,bitmap is null.");
                    }
                    ...
                    AbsImageCallback imgCb = new AbsImageCallback() {
                        ...
                        protected void doSuccessAction(Bitmap bitmap) {
                            if (null != bitmap) {
                                WmCanvasJNI.bindTexture(canvasId, bitmap, id, GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE);
                            } else {
                                WmLog.d("bitmap is null in teximage2D.");
                            }
    ...
    public void setLogLevel(int level) {
        ...
        WmCanvasJNI.setLogLevel(logLevel);
    }
        ...
        if (!TextUtils.isEmpty(path)) {
            try {
                if (path.startsWith("data:image")) {
                    ...
                    if (bmp != null) {
                        WmCanvasJNI.texSubImage2D(canvasid, bmp, 0, target, level, xoffset, yoffset, format, type);
                    } else {
                        WmLog.d("[texSubImage2D] decode base64 texture failed,bitmap is null.");
                    }
                } else {
                    AbsImageCallback imgCb = new AbsImageCallback() {
                        @Override
                        protected void doSuccessAction(Bitmap bitmap) {
                            if (null != bitmap) {
                                WmLog.d("[texSubImage2D] start to bindtexture in 3dmodule.");
                                WmCanvasJNI.texSubImage2D(canvasid, bitmap, 0, target, level, xoffset, yoffset, format, type);
                            } else {
                                WmLog.d("[texSubImage2D] bitmap is null.");
                            }
                        }
                    };
        ...
        if (!TextUtils.isEmpty(path)) {
            try {
                if (path.startsWith("data:image")) {
                    Bitmap bmp = handleBase64Texture(path.substring(path.indexOf("base64,") + "base64,".length()));
                    if (bmp != null) {
                        WmCanvasJNI.bindTexture(canvasId, bmp, 0, target, level, internalformat, format, type);
                    } else {
                        WmLog.d("decode base64 texture failed,bitmap is null.");
                    }
                } else {
                    ...
                    AbsImageCallback imgCb = new AbsImageCallback() {
                        ...
                        protected void doSuccessAction(Bitmap bitmap) {
                            if (null != bitmap) {
                                WmCanvasJNI.bindTexture(canvasId, bitmap, 0, target, level, internalformat, format, type);
                            } else {
                                WmLog.d("bitmap is null in teximage2D.");
                            }
                        }
    ...
    public void preLoadImage(JSONArray arrayPara, final JSCallback callback) {
        ...
        try {
            ...
            result.putString("error", "invalid input param. specify an json array which length is 2, and index 0 is url to load, index 1 is image id.");
            WmLog.d(TAG, "error parse preload image data:" + e.getMessage());
            invokeCallback(callback, result);
            return;
        }
        ...
        try {
            ...
            WmImageLoadInfo imgInfo = mImageIdCache.get(url);

            if (null == imgInfo) {
                imgInfo = new WmImageLoadInfo();
                mImageIdCache.put(url, imgInfo);
            }
            if (imgInfo.status.get() == WmImageLoadInfo.IDLE) {
                imgInfo.status.set(WmImageLoadInfo.LOADING);
                imgInfo.id = imageId;
                ...
                mImageLoader.load(getContext(), url, new IWmImageLoader.ImageCallback() {
                    @Override
                    public void onSuccess(Bitmap bitmap) {
                        if (null != bitmap) {
                            WmImageLoadInfo imageInfo = mImageIdCache.get(urlFinal);
                            ...
                            imageInfo.status.set(WmImageLoadInfo.LOADED);
                            try {
                                ...
                                imageInfo.status.set(WmImageLoadInfo.IDLE);
                                invokeCallback(callback, resultMap);
                            }
                        } else {
            ...
            } else if (WmImageLoadInfo.LOADING == imgInfo.status.get()) {
                ...
                callbacks.add(callback);
            } else if (WmImageLoadInfo.LOADED == imgInfo.status.get()) {
                resultMap.putInt("id", imageId);
                resultMap.putString("url", url);
