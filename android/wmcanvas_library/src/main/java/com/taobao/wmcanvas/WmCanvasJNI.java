...
public class WmCanvasJNI {
    ...
    private static void loadInternal(String scene) {
        ...
        WmLog.i("CANVAS", "WmCanvasJNI init from " + scene);
        try {
    ...
    public static void setFontFamilies() {
        WmFontConfigParser parser = new WmFontConfigParser();
        String fallbackFont = parser.getFallbackFont();
        String systemFontLocation = parser.getSystemFontLocation();
        WmCanvasJNI.setFallbackFont(fallbackFont, systemFontLocation);

        HashMap<List<String>, List<String>> fontFamilies = parser.getFontFamilies();
        if (fontFamilies != null) {
            WmLog.d("CANVAS", "setFontFamilies() fontFamilies.size() " + fontFamilies.size());
        } else {
            WmLog.d("CANVAS", "setFontFamilies() error! fontFamilies is empty");
        }
        ...
        if (fontFamilies != null) {
            ...
            while (it.hasNext()) {
                ...
                WmCanvasJNI.addFontFamily(fontNameArray, fontFileArray);
            }
        }
        ...
        WmCanvasJNI.addFallbackFontFamily(FallbackFileArray);
    }
    ...
    public static void registerWXCallNativeFunc(Context context) {
         ...
         * Weex's customized JavascriptCore has been injected a special handle to deal WmCanvas' call. All calls can directly send to JavascriptCore, avoiding JNI calls, which significantly improves performance.
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            WmLog.i("start to test load WmCanvas library,path=" + libraryPath);
        }
        WmCanvasJNI.registerCallback(libraryPath, Build.VERSION.SDK_INT);
    }
