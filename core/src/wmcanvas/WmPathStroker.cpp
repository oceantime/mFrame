...
void GPathStroker::DrawLineJoin(WmCanvasContext* context, float halfLineWidth, GPathOutLine& lineOne, GPathOutLine& lineTwo,
    ...
    if (context->LineJoin() == LINE_JOIN_ROUND) {
        ...
        if (wmcanvas::IsTrianglePointsValid(lineTwo.from, joinP1, joinP2, PATH_MIN_VALID)) {
...
void GPathStroker::DrawArcForCapOrJoin(WmCanvasContext *context, float halfLineWidth,
    ...
    // filter nan pint
    if (wmcanvas::IsNanPoint(center) || wmcanvas::IsNanPoint(p1) || wmcanvas::IsNanPoint(p2)) {
        return;
    }
...
void GPathStroker::DrawMiterJoin(WmCanvasContext *context, float halfLineWidth,
    ...
    if (wmcanvas::IsNanPoint(p1) || wmcanvas::IsNanPoint(p2) ||
            wmcanvas::IsNanPoint(center) || wmcanvas::IsNanPoint(miterPoint)) {
        return;
    }
