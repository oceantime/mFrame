...
#include "wmcanvas/GL/WmShaderManager.h"
...
GCanvasWeex::PutImageData(
    ...
    wmcanvas::Base64DecodeBuf(reinterpret_cast<char *>(pixels), imageData, dataLength);
...
GCanvasWeex::GetImageData(int
    ...
    wmcanvas::FlipPixel(pixels, w, h);

    if (base64Encode) {
        pixelsData.resize(wmcanvas::Base64EncodeLen(buf_size));
...
const char *GCanvasWeex::CallNative(int type, const std::string &args) {
    ...
    if (sync == SYNC) {
        ...
        wmcanvas::waitUtilTimeout(&mSyncSem, GCANVAS_TIMEOUT);
