...
void RegisterCallNativeCallback_belowN() {
    ...
    RegisterFunc func = (RegisterFunc) dlsym(handle, "Inject_WmCanvasFunc");
    if (!func) {
        LOG_D("load Inject_WmCanvasFunc failed,error=%s\n", dlerror());
        dlclose(handle);
        return;
    }
...
void executeCallbacks(JNIEnv *je, jstring contextId) {
    ...
    if (callback) {
        jclass cls = je->FindClass("com/taobao/gcanvas/WmCanvas");
...
JNIEXPORT void JNICALL Java_com_honghu_wmcanvas_WmCanvasJNI_bindTexture(
    ...
    LOG_D("bindtexture in WmCanvasJNI.");
...
JNIEXPORT void JNICALL Java_com_honghu_wmcanvas_WmCanvasJNI_texSubImage2D(
    ...
    LOG_D("texSubImage2D in WmCanvasJNI.");
